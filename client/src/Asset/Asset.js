...
const Asset = () => {
  ...
  useEffect(() => {
    fetch(
      `http://localhost:3001/_api/web/lists/GetByTitle('Assets')/items(${params.id})`,
      { credentials: 'include' }
    )
      .then((res) => res.json())
      .then((data) => {
          ...
          <div className="flex flex-row gap-5 h-3/12 mb-4 justify-center">
            ...
            <div className="w-1/4 block rounded-lg bg-bluer/25 border border-black text-center overflow-hidden flex flex-row">
              ...
              <div>
                ...
                <div>{`${currAsset.ThreatType.toUpperCase()} ${
                  currAsset.Equipment
                } as ${currAsset.Threat}`}</div>
